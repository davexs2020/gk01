class Table {
  ...
  update( value ) {
    ...
    row.children[0].innerHTML = value[s].kpcid;
    row.children[1].innerHTML = value[s].kpname;
    row.children[2].innerHTML = value[s].kpfund;
  }
  ...
  set data( value ) {
    ...
    for( let s = 0; s < value.length; s++ ) {
      ...
      row.children[0].innerHTML = value[s].psid;
      row.children[1].innerHTML = value[s].psname;
      row.children[2].innerHTML = '<a href="#">Keeper</a>';
  ...
  get data() {
    ...
    for( let r = 0; r < this.list.children.length; r++ ) {
      let stock = {
        kpcid:  this.list.children[r].children[0].innerHTML.trim(),
        kpname: this.list.children[r].children[1].innerHTML.trim(),
        kpfund: parseFloat( this.list.children[r].children[2].innerHTML ),
      };
      results.push( stock );
    }
